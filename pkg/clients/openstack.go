...
import (
	...
	"github.com/subhransusekhar/provider-openstack/apis/v1alpha1"
)

// GetAuthInfo returns the necessary authentication information that is necessary
// to use when the controller connects to OpenStack API in order to reconcile
// the managed resource.
func GetAuthInfo(ctx context.Context, c client.Client, mg resource.Managed) (identityEndpoint string, projectID string, userName string, token string, err error) {
	...
	identityEndpoint := pc.Spec.identityEndpoint
	if identityEndpoint == nil {
		return "", errors.New("no identityEndpoint reference was provided")
	}

	projectID := pc.Spec.projectID
	if projectID == nil {
		return "", errors.New("no projectID reference was provided")
	}

	userName := pc.Spec.userName
	if userName == nil {
		return "", errors.New("no userName reference was provided")
	}

	return string(identityEndpoint), string(projectID), string(userName), string(s.Data[ref.Key]), nil
}
